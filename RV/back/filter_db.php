<?php
    ...
    if($diff != 'allDifficulty' and $keyword != '' and $topic != 'allTopics'){
        $filter = "SELECT Q.QID, QText FROM QUESTION Q, QUES_KEY QK, QUES_TYPE QT WHERE Q.QID = QK.QID AND Q.QID = QT.QID AND QText LIKE '%$keyword%' AND Diff='$diff' AND Topic='$topic';";
    }
    elseif($diff != 'allDifficulty' and $keyword != '' and $topic == 'allTopics'){
        $filter = "SELECT Q.QID, QText FROM QUESTION Q, QUES_KEY QK WHERE Q.QID = QK.QID AND Diff='$diff' AND QText LIKE '%$keyword%';";
    }
    elseif($diff != 'allDifficulty' and $keyword == '' and $topic != 'allTopics'){
        $filter = "SELECT Q.QID, QText FROM QUESTION Q, QUES_TYPE QT WHERE Q.QID = QT.QID AND Diff='$diff' AND Topic='$topic';";
    }
    elseif($diff == 'allDifficulty' and $keyword != '' and $topic != 'allTopics'){
        $filter = "SELECT Q.QID, QText FROM QUESTION Q, QUES_KEY QK, QUES_TYPE QT WHERE Q.QID = QK.QID AND Q.QID = QT.QID AND QText LIKE '%$keyword%' AND Topic='$topic';";
    }
    elseif($diff == 'allDifficulty' and $keyword == '' and $topic !='allTopics'){
        $filter = "SELECT Q.QID, QText FROM QUESTION Q, QUES_TYPE QT WHERE Q.QID = QT.QID AND Topic='$topic';";
    }
    elseif($diff == 'allDifficulty' and $keyword != '' and $topic =='allTopics'){
        $filter = "SELECT Q.QID, QText FROM QUESTION Q, QUES_KEY QK WHERE Q.QID = QK.QID AND QText LIKE '%$keyword%';";
    }  
    elseif($diff != 'allDifficulty' and $keyword == '' and $topic == 'allTopics'){
